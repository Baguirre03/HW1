...
/*
 * *** Ben Aguirre / COMP 400C-001 SP25 ***
 *
...
public class HW1 {
    ...
    static class LinkedList {
        ...
        public void removeElementsLT(int ltValue) {
            // decalre head of node into a current variableut
            Node cur = this.head;
            Node prev = null;
            // while there is a next node loop through
            // since this is a sorted linkedlist we do not need to remove each node
            // instead we find the first node that is greater than value
            // then splice from there
            boolean nodeFound = false;
            while (cur.next != null && !nodeFound) {
                if (cur.data >= ltValue) {
                    // remove node
                    // if first node is greater than, dont remove anything
                    if (prev != null) {
                        // set the head to cur (splicing node)
                        this.head = cur;
                    }
                    // end loop with while loop condition
                    nodeFound = true;
                }
                // move pointers
                prev = cur;
        ...
        public void removeElement(int value) {
            ...
            // while there is a next node loop through
            // since this is a sorted linkedlist we do not need to remove each node
            // keep track of current and previous pointers and a lessThan boolean
            // once we get a number higher than value we can end loop
            // since it is a sorted list
            boolean lessThan = false;
            while (cur.next != null && !lessThan) {
                if (cur.data > value)
                    lessThan = true;
                if (cur.data == value) {
                    // remove node
                    ...
                    if (prev == null) {
                        ...
                        // else set prev.next to cur.next to remove current node from list
                        prev.next = cur.next;
                    }
                }
                // move pointers
    ...
    static class Stacks {
        ...
        public static boolean isPalindrome(String input) {
            ...
            // find half of the input
            int half = input.length() / 2;
            // declare an index to keep track of current char
            int indx = 0;
            // while index is less than half, of the input
            while (indx < half) {
                // push the character into the stack
                stack.push(input.charAt(indx));
                indx += 1;
            }
            // if the length is odd, then the middle character does not matter,
            // increase index by one
            if (!(input.length() % 2 == 0)) {
                indx += 1;
            }

            // compare the second half of the input
            while (indx < input.length()) {
                // if the top of the stack element equals the current index, then it matches
                // palindrome wise
                if (stack.lastElement().equals(input.charAt(indx))) {
                    // when that happens pop from stack
                    stack.pop();
                }
            ...
            // finally if there is still chars in the stack then it is NOT palindrome,
            // if it is empty then all characters matched on both sides and palindrome is
            // true
            return stack.size() == 0;
        ...
        public static int findLargestK(Stack<Integer> stack, int k) {
            // implement temp stack
            Stack<Integer> tmpStack = new Stack<>();
            // if not stack return 0
            if (stack.size() == 0)
                return 0;
            // simplified: return the highest index for the integer K sitting in the stack
            // keep track of current index to return
            int curIndex = stack.size();
            // if we find the number we want to stop while loop
            boolean found = false;
            // while we have the stack and num not found
            while (stack.size() > 0 && !found) {
                // grab the top number
                int topNumber = stack.peek();
                // push the top number into the temporary stack
                tmpStack.push(stack.pop());
                // if top number == k then stop while loop
                if (topNumber == k) {
                    found = true;
                }

                // decrease index to match correct el
                curIndex -= 1;
            }

            // push the tmpStack back into the original stack to keep original state
            while (tmpStack.size() > 0) {
                stack.push(tmpStack.pop());
            }

            // return correct index of highest K element in stack
            return curIndex;
        }
